...
func NewUserHandler( session *session.Cookiehandler , cs User.UserService )  *UserHandler {
	return &UserHandler{
...
func (userh *UserHandler)   UpdateUserProfile(response http.ResponseWriter  , request *http.Request){
	...
	var chage int 
	user.Username = func()string { if (in.Username != "") && (in.Username != user.Username) && (len(in.Username)>3) {
		chage = 1
		return in.Username 
		}
		return user.Username }()
	user.Bio = func()string { if (in.Bio!= "") && in.Bio != user.Bio {
		if chage ==1 {
	...
	if chage==4 || chage==1 || chage==2{
		...
		if user == nil {ret.Message = translation.Translate(lang , "Internal Server ERROR ")
			response.Write(Helper.MarshalThis(ret))
			return 
		}
		ret.Success= true
